...
if [ "${SKIP_BUILD}" == "NO" ]; then
    #pixterm : Draw images in your ANSI terminal with true color
     export BIN="pixterm"
     export SOURCE_URL="https://github.com/eliukblau/pixterm"
     echo -e "\n\n [+] (Building | Fetching) ${BIN} :: ${SOURCE_URL} [$(TZ='UTC' date +'%A, %Y-%m-%d (%I:%M:%S %p)') UTC]\n"
      #Build (alpine-musl)
       ...
       docker run --privileged --net="host" --name "alpine-builder" "azathothas/alpine-builder:v2025.01.02" \
        ...
        #Build
         git clone --quiet --filter "blob:none" "https://github.com/eliukblau/pixterm" && cd "./pixterm"
         GOOS="linux" GOARCH="amd64" CGO_ENABLED="1" CGO_CFLAGS="-O2 -flto=auto -fPIE -fpie -static -w -pipe" go build -v -trimpath -buildmode="pie" -ldflags="-s -w -buildid= -linkmode=external -extldflags '\''-s -w -static-pie -Wl,--build-id=none'\''" "./cmd/pixterm"
        #strip & info
