...
if [ "${SKIP_BUILD}" == "NO" ]; then
     ...
     echo -e "\n\n [+] (Building | Fetching) ${BIN} :: ${SOURCE_URL} [$(TZ='UTC' date +'%A, %Y-%m-%d (%I:%M:%S %p)') UTC]\n"
       #Build (upstream fork is gone; build the real upstream directly)
       pushd "$($TMPDIRS)" >/dev/null 2>&1 && git clone --quiet --filter "blob:none" "https://github.com/zer0yu/anew" && cd "./anew"
       ...
       cargo build --target "$RUST_TARGET" --release --jobs="$(($(nproc)+1))" --keep-going ; cp "./target/$RUST_TARGET/release/anew" "$BINDIR/anew-rs" ; popd >/dev/null 2>&1
fi
#-------------------------------------------------------#
