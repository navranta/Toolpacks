...
if [ "${SKIP_BUILD}" == "NO" ]; then
     ...
     export BIN="inscope" #Name of final binary/pkg/cli, sometimes differs from $REPO
      export SOURCE_URL="https://github.com/pkgforge-security/inscope" #github/gitlab/homepage/etc for $BIN
     echo -e "\n\n [+] (Building | Fetching) ${BIN} :: ${SOURCE_URL} [$(TZ='UTC' date +'%A, %Y-%m-%d (%I:%M:%S %p)') UTC]\n"
      #Build
       pushd "$($TMPDIRS)" >/dev/null 2>&1 && mkdir -p "./inscope" && cd "./inscope"
        curl -qfsSLJO "https://raw.githubusercontent.com/pkgforge-security/inscope/main/main.go"
        go mod init "github.com/pkgforge-security/inscope" ; go mod tidy
