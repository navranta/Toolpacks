...
if [ "${SKIP_BUILD}" == "NO" ]; then
    #gitlab-cli : Official Gitlab CLI
     export BIN="gitlab-cli"
     export SOURCE_URL="https://gitlab.com/gitlab-org/cli"
     echo -e "\n\n [+] (Building | Fetching) ${BIN} :: ${SOURCE_URL} [$(TZ='UTC' date +'%A, %Y-%m-%d (%I:%M:%S %p)') UTC]\n"
      #Build (alpine-musl)
       ...
       docker run --privileged --net="host" --name "alpine-builder" "azathothas/alpine-builder:v2025.01.02" \
        ...
        #Build
         git clone --quiet --filter "blob:none" "https://gitlab.com/gitlab-org/cli" && cd "./cli"
         GOOS="linux" GOARCH="amd64" CGO_ENABLED="1" CGO_CFLAGS="-O2 -flto=auto -fPIE -fpie -static -w -pipe" go build -v -trimpath -buildmode="pie" -ldflags="-s -w -buildid= -linkmode=external -extldflags '\''-s -w -static-pie -Wl,--build-id=none'\''" "./cmd/glab"
        #strip & info
         ...
         cp "/build-bins/glab" "/build-bins/gitlab-cli"
