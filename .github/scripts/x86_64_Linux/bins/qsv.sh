...
if [ "${SKIP_BUILD}" == "NO" ]; then
    #qsv: Blazing-fast Data-Wrangling toolkit 
     export BIN="qsv"
     export SOURCE_URL="https://github.com/dathere/qsv"
     echo -e "\n\n [+] (Building | Fetching) ${BIN} :: ${SOURCE_URL} [$(TZ='UTC' date +'%A, %Y-%m-%d (%I:%M:%S %p)') UTC]\n"
      #Build
       ...
       docker run --privileged --net="host" --name "alpine-builder" "azathothas/alpine-builder:v2025.01.02" \
        ...
        #Build
         git clone --filter "blob:none" --quiet "https://github.com/dathere/qsv" && cd "./qsv"
         echo -e "\n[+] Target: ${RUST_TARGET}\n"
         echo -e "\n[+] Flags: ${RUSTFLAGS}\n"
         sed "/^\[profile\.release\]/,/^$/d" -i "./Cargo.toml" ; echo -e "\n[profile.release]\nstrip = true\nopt-level = 3\nlto = true" >> "./Cargo.toml"
         rm rust-toolchain* 2>/dev/null
         #https://github.com/dathere/qsv/blob/master/docs/FEATURES.md
         #polars makes it slooow
         cargo build --target "${RUST_TARGET}" --features "feature_capable,to" --release --jobs="$(($(nproc)+1))" --keep-going
